...
func New(ctx context.Context, buildTime, gitCommit, version string, cfg *config.Config, deps Dependencies) (*Download, error) {
	...
	if cfg.IsPublishing {
		router.Path("/downloads-new/{path:[a-zA-Z0-9_\\.\\-\\/]+}").HandlerFunc(authMiddleware.Require("static-files:read", downloadHandler))
	} else {
		router.Path("/download-new/{path:[a-zA-Z0-9_\\.\\-\\/]+}").HandlerFunc(downloadHandler)
	}
